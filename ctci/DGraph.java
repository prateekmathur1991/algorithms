...
public class DGraph	{
	...
	public void BFS(String r, boolean verbose)	{
		if (!hasVertex(r))	{
		...
		while (queue.size() != 0)	{
			...
			while (itr1.hasNext())	{
				Vertex v = itr1.next();
				if ("w".equals(v.color) && v.marked == false)	{
					if (verbose) {
	...
	public boolean hasRoute(String source, String destination)	{
		if (!hasVertex(source))	{
			System.err.println("SOURCE VERTEX NOT PRESENT IN GRAPH");
			return false;
		}

		if (!hasVertex(destination))	{
			System.err.println("DESTINATION VERTEX NOT PRESENT IN GRAPH");
			return false;
		}
		
		Vertex dest = allVertices.get(destination);
		
		if (hasEdge(source, destination))	{
			return true;
		}
		
		this.BFS(source, false);
